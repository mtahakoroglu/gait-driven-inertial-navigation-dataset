...
img5 = cv2.imread(trajectoryLSTM)
img6 = cv2.imread(zvlabelsLSTM)
...
if img5.shape[0] > img6.shape[0]:
    s = img6.shape[0] / img5.shape[0]
    img5 = cv2.resize(img5, (int(s*img5.shape[1]), int(s*img5.shape[0])), 0)
else:
    s = img5.shape[0] / img6.shape[0]
    img6 = cv2.resize(img6, (int(s*img6.shape[1]), int(s*img6.shape[0])), 0)
...
imgCombined3 = np.zeros((img5.shape[0], img5.shape[1]+img6.shape[1], 3), np.uint8)
imgCombined3[:, 0:img5.shape[1]] = img5
imgCombined3[:, img5.shape[1]:img5.shape[1]+img6.shape[1]] = img6
...
cv2.imwrite(f"exp{expNumber}_lstm.jpg", imgCombined3, [cv2.IMWRITE_JPEG_QUALITY, 50])
