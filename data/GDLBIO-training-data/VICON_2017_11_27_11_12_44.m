...
expID = 27; % VICON room experiments of PyShoe dataset
%% import & extract data
expname = '2017-11-27-11-12-44';
...
%% plot pyshoe (lstm) and gt trajectory
figure(expID+2); 
plot(pyshoeTrajectory(:,1), pyshoeTrajectory(:,2), 'b-', 'linewidth', 1.2); hold on;
plot(gt(:,1), gt(:,2), 'r-', 'linewidth', 1.2); grid on;
plot(pyshoeTrajectory(strideIndex,1), pyshoeTrajectory(strideIndex, 2), 'bo', 'markerface', 'b');
axis equal;
grid on; set(gca, 'gridlinestyle', '--');
xlabel('x [m]'); ylabel('y [m]');
titleText = sprintf('Exp#%i (%s) - PyShoe (LSTM)', expID, expname);
title(titleText, 'FontWeight','normal');
legend('PyShoe (LSTM)', 'GT');